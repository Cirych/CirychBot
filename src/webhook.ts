...
export class WebHook {
    ...
    public setWebhook() {
        for (let token of this.bots.keys()) {
            ...
            this.requestPromise(options, webhook).then(function(data: any) {
    ...
    public requestPromise(options: {}, request: string) {
    ...
    private requestListener(req: http.ServerRequest, res: http.ServerResponse) {
        ...
        if (!this.bots.has(token)) {
            ...
            req.on('end', () => {
                try {
                    let reply: IReply|boolean = this.bots.get(token).call(JSON.parse(body));
